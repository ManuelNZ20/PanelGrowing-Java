...
public class PanelGlowingGradient extends JComponent {
    ...
    private void createImageRender() {
        ...
        if (witdh > 0 && height > 0) {
            ...
            imageRender = new BufferedImage(witdh, height, BufferedImage.TYPE_INT_ARGB);
            Graphics2D g2 = imageRender.createGraphics();
            g2.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
            //Draw shadow left
            g2.drawImage(createShadow(witdh, height, inset, gradientColor1, true), 0, 0, null);
            //Draw shadow right
            g2.drawImage(createShadow(witdh, height, inset, gradientColor2, false), 0, 0, null);
            Area area = new Area(new Rectangle2D.Double(inset.left, inset.top, witdh - inset.left - inset.right, height - inset.top - inset.bottom));

            g2.setColor(getBackground());
            g2.fill(area);
            area.subtract(new Area(new Rectangle2D.Double(witdh / 2, 0, witdh / 2, height)));
    ...
    public void setBounds(int x, int y, int width, int height) {
        ...
        createImageRender();
        //Update or create new image render when component has resize or change size - Actualice o cree una nueva representación de imagen cuando el componente cambie de tamaño o cambie de tamaño
